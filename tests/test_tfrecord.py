...
def tiff_image(tmp_path):
    numpy_image = np.random.rand(64, 32, 3)
    filename = str(tmp_path / 'test.tiff')
    image_writer = tiff.TiffWriter(filename, 64, 32, 3, data_type=gdal.GDT_Float32)
    for i in range(3):
...
def test_save_tfrecord(tiff_image, tmp_path):
    ...
    tfrecord.write_tfrecord_image(image.read(), image_writer,
                                  0, 0, image.height(), image.width(), image.num_bands())
    image_writer.close()
    ...
    for value in raw_dataset.take(1):
        ...
        assert np.array_equal(value, tiff_image[0])
