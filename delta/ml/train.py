...
def _prep_datasets(ids, tc, model):
    ...
    if in_shape[1] is None and out_shape[1] is None:
        ...
        if out_shape[1] != in_shape[1] or out_shape[2] != in_shape[2]:
            ids.set_chunk_output_shapes(None, (out_shape[1], out_shape[2]))

    chunk_shape = in_shape[1:3]
    output_shape = out_shape[1:3]
    ...
    if tc.validation:
        if tc.validation.from_training:
            ...
            if not vimg:
                ...
                # TODO: use correctly in imagerydataset
                stride = tc.chunk_stride if tc.chunk_stride is not None else output_shape
                if vlabel:
                    vimagery = ImageryDataset(vimg, vlabel, output_shape, chunk_shape,
                                              tile_shape=config.io.tile_size(), chunk_stride=stride)
                else:
                    vimagery = AutoencoderDataset(vimg, chunk_shape, tile_shape=config.io.tile_size(),
                                                  chunk_stride=stride)
...
def train(model_fn, dataset : ImageryDataset, training_spec):
    ...
    (ds, validation) = _prep_datasets(dataset, training_spec, model)
